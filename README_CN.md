...
- Seeed RoboController 提供了舵机 ID 扫描、力矩关闭、中位写入 2048、中位测试、LeRobot 风格整机校准等工具。OpenNeck 已把其中需要的 ID 扫描、改 ID、2048 中位校准做成本地脚本；OpenNeck 不生成 LeRobot 校准文件，而是把自己的 yaw/pitch 物理正前方和安全极限保存到 `active_vision_config.json`。
...
- `2048 中位校准` 是舵机内部校准：把当前机械姿态写成舵机自己的中位值 2048。这个步骤由本仓库的 `tools/calibrate_servo_middle.py` 完成，OpenNeck 平时不需要重复做。
...
4. 默认约定 `yaw_id=1`、`pitch_id=2`。如果你换过舵机或总线里还有其他舵机，先用本仓库脚本扫描/修改 ID。
...
- 读数异常或之前发生过撞限位：先断电重上电；仍异常时，用本仓库脚本做 2048 中位校准。
...
脚本会默认关闭力矩，提示你手动把 yaw、pitch 放到希望的机械中位，然后把当前位置写入为 2048，并执行一次移动到 2048 的验证。

验证标准：脚本移动到 2048 后，云台应该回到预期的物理中位附近。
